...
- **T6-UX01 — Léo, assistant IA (demande utilisateur) :** Léo incarne l’assistance à la préparation dans Localeo Animation. Le bouton générique « Préparer avec l’IA » est retiré ; la carte Chasse au trésor commerçante porte la mention « Assisté par Léo » dans le catalogue et le choix de modèle. Le bouton « Continuer » ouvre la préparation guidée existante. Pour préserver T2-D02, le Passeport conserve ses deux modes via un choix « Je prépare moi-même » / « Avec Léo, assistant IA », manuel par défaut, puis le même bouton Continuer. Tombola garde sa configuration manuelle.
